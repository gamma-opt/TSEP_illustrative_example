function single_level_problem_generation(ip::initial_parameters)    
    ...
    @constraint(single_level_problem, [i = 1:ip.num_prod],
        sum(ip.conv.investment_costs[n,i,e] * g_conv_plus[n,i,e] for  e in 1:ip.num_conv, n in 1:ip.num_nodes) <= ip.conv.budget_limits[i]
    )
    ...
    @constraint(single_level_problem, [i = 1:ip.num_prod],
        sum(ip.vres.investment_costs[n,i,e] * g_VRES_plus[n,i,e] for  e in 1:ip.num_VRES, n in 1:ip.num_nodes) <= ip.vres.budget_limits[i]
    )
    ...
    @constraint(single_level_problem,
        sum(ip.transm.investment_costs[n,m] * l_plus[n,m] for n in 1:ip.num_nodes, m in 1:ip.num_nodes) - ip.transm.budget_limit <= 0 
    )
...
function bi_level_problem_generation(ip::initial_parameters, market::String)    
    ...
    #set_optimizer_attribute(bi_level_problem, "Method", 1)
    ...
    @objective(bi_level_problem, Max, 
        ...
        for n in 1:ip.num_nodes)/obg_scaling_factor
    )
